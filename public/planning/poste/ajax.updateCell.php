...
if (is_numeric($perso_id) && $perso_id == 0) {
    // Tout barrer
    if ($barrer == 1 && $tout) {
...
for ($i=0;$i<$counter;$i++) {
  ...
  // Distinction des agents volants
    if ($config['Planning-agents-volants'] && in_array($tab[$i]['perso_id'], $agents_volants)) {
    ...
    $tab[$i]['color'] = null;
    if (!empty($config['Affichage-Agent']) && $tab[$i]['perso_id'] == $_SESSION['login_id']) {
    ...
    foreach ($absences as $absence) {
        if ($absence["perso_id"] == $tab[$i]['perso_id'] && $absence['debut'] < $date . " " . $fin && $absence['fin'] > $date . " " . $debut) {

            if ($config['Absences-Exclusion'] == 1 && $absence['valide'] == 99999 || $config['Absences-Exclusion'] == 2) {
                // Nothing changes. If absent = 1 because manually crossed out, absent must remain equal to 1.
            } elseif ($absence['valide'] > 0 || $config['Absences-validation'] == 0) {
                $tab[$i]['absent'] = 1;
                break;  // Garder le break à cet endroit pour que les absences validées prennent le dessus sur les non-validées
            } elseif ($config['Absences-non-validees'] && $tab[$i]['absent'] != 1) {
                $tab[$i]['absent'] = 2;
            }
        }
    }
...
if ($config['Conges-Enable']) {
    include __DIR__ . "/../../conges/ajax.planning.updateCell.php";
}
