...
class WorkinghourImportFromAgentCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $numberOfWeeks = $entityManager->getRepository(Config::class)->findOneBy(['nom' => 'nb_semaine'])->getValue();
        ...
        foreach ($agents as $agent) {
            $workingHours = $agent->getWorkingHours();
            $isEmpty = true;
        ...
        $config = $entityManager->getRepository(Config::class)->findOneBy(['nom' => 'PlanningHebdo']);
        $config->setValue('1');
        $entityManager->flush();
