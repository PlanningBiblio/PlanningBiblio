...
class PresentSet
{
    ...
    public function all(): array
    {
        ...
        if ($config['PlanningHebdo']) {
            $tempsPlanningHebdo = $this->getPlanningHebdo($date);
        }

        $presents = array();
        foreach ($this->db->result as $elem) {
            // Exclude agents who are not working on the request site
            if ($config['Multisites-nombre'] > 1 && $this->site != 0 ) {
                $agentSites = json_decode($elem['sites']);
                if (!is_array($agentSites) || !in_array($this->site, $agentSites)) {
                    continue;
                }
            }
            ...
            if ((!empty($temps) and array_key_exists($jour, $temps)) && ($temps[$jour][0] || $temps[$jour][2])) {
                $heures=$temps[$jour];
            }

            // S'il y a des horaires correctement renseignés
            $siteAgent=null;
            if ($heures && !in_array($elem['id'], $absents)) {
                ...
                $horaires=null;
                if (!$heures[1] && !$heures[2]) {		// Pas de pause le midi
                    $horaires=heure2($heures[0])." - ".heure2($heures[3]);
                } elseif (!$heures[2] && !$heures[3]) {	// matin seulement
                    $horaires=heure2($heures[0])." - ".heure2($heures[1]);
                } elseif (!$heures[0] && !$heures[1]) {	// après midi seulement
     ...
     * @return mixed[]
     */
    private function getPlanningHebdo($date): array
    {
