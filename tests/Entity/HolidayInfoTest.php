...
class HolidayInfoTest extends KernelTestCase
{
    public function testGettersAndSetters(): void
    {
        ...
        $entryDate = new \DateTime('2026-02-10 09:00:00');
        ...
        $this->assertSame($start->format('Y-m-d'), $holiday->getStart()->format('Y-m-d'));
        $this->assertSame($end->format('Y-m-d'), $holiday->getEnd()->format('Y-m-d'));
        $this->assertSame('Congés estivaux', $holiday->getComment());
        $this->assertSame($entryDate->format('Y-m-d H:i:s'), $holiday->getEntryDate()->format('Y-m-d H:i:s'));
    }
}
